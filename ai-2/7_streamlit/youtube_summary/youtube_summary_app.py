...
from youtube_transcript_api.errors import NoTranscriptFound, TranscriptsDisabled
...
def summarize_youtube_video(video_url, select_lang, trans_method):
    if select_lang == '영어':
        lang = 'en'
    else:
        lang = 'ko'
    ...
    div_num, divide_ty_transcript = divide_text(yt_transcript, token_num)
    
    st.write('유튜브 동영상 내용 요약 중입니다. 잠시만 기다려 주세요...')

    summaries = []
    
    for divide_yt_transcript in divide_ty_transcript:
        summary = my_text_sum.summary_text(divide_yt_transcript, lang)
