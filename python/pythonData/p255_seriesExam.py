...
print('# values 속성으로 이용한 요소들의 값 확인')
print(member.values)
print('-' * 50)

print('# index 속성으로 이용한 색인 객체 확인')
print(member.index)
print('-' * 50)

member.plot(kind='bar', rot=40, ylim=[0, member.max() + 10], figsize=(10, 6), use_index=True, grid=False, table=False, color=['r', 'b', 'g', 'y'])
plt.xlabel('학생이름')
plt.ylabel('점수')
plt.title('학생별 시험 점수')
