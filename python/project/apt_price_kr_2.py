...
SUBWAY_DATA = [
    ...
    (2011, "증심사입구역", "광주광역시"), (2010, "부산 4호선", "부산"),
...
def run_analysis(station_name, region_name, open_year):
    ...
    election_dates_parl = [datetime.strptime(e[0], "%Y-%m-%d") for e in ELECTIONS if e[1] == "총선"]
    ...
    df["연월"] = df["계약일"].dt.to_period("M").dt.to_timestamp()
    ...
    plt.axvline(open_date, color="red", linestyle="--", label="지하철 개통")
    for i, p in enumerate(policy_dates):
        plt.axvline(p, color="blue", linestyle=":", label="정부정책" if i == 0 else "")
    for i, e in enumerate(election_dates_pres):
        plt.axvline(e, color="green", linestyle="-.", label="대통령 선거" if i == 0 else "")
    for i, e in enumerate(election_dates_parl):
        plt.axvline(e, color="lime", linestyle="-.", label="총선" if i == 0 else "")
    for i, (d, t) in enumerate(interest_rate_events):
        color = "black" if t == "인상" else "gray"
        style = "-" if t == "인상" else "--"
        plt.axvline(d, color=color, linestyle=style, label=f"금리 {t}" if i == 0 else "")
