        ...
        for item in items.findall("item"):
            try:
                results.append({
                    ...
                    "전용면적(m²)": float(item.findtext("excluUseAr", 0)),
...
def run_analysis(station_name, region_name, open_year):
    ...
    pres_elections = [datetime.strptime(e[0], "%Y-%m-%d") for e in ELECTIONS if e[1] == "대통령"]
    parl_elections = [datetime.strptime(e[0], "%Y-%m-%d") for e in ELECTIONS if e[1] == "총선"]
    rate_events = [(datetime.strptime(r[0], "%Y-%m-%d"), r[1]) for r in INTEREST_RATES]
    ...
    if df.empty:
        print(f"⚠️ 데이터 없음: {station_name}")
        return

    def label_period(d):
        if d < datetime(open_year - 2, 1, 1): return "개통 전"
        elif d < open_date: return "공사 중"
        else: return "개통 후"
    ...
    for i, p in enumerate(policy_dates):
        plt.axvline(p.strftime("%Y-%m"), color="blue", linestyle=":", label="정부 정책" if i == 0 else "")
    for i, e in enumerate(pres_elections):
        plt.axvline(e.strftime("%Y-%m"), color="green", linestyle="-.", label="대통령 선거" if i == 0 else "")
    for i, e in enumerate(parl_elections):
        plt.axvline(e.strftime("%Y-%m"), color="lime", linestyle="-.", label="총선" if i == 0 else "")

    rate_labels = {"인상": False, "인하": False}
    for d, t in rate_events:
        color = "black" if t == "인상" else "gray"
        style = "-" if t == "인상" else "--"
        label = f"금리 {t}" if not rate_labels[t] else ""
        rate_labels[t] = True
        plt.axvline(d.strftime("%Y-%m"), color=color, linestyle=style, label=label)
